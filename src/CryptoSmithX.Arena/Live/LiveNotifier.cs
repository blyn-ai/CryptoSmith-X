...
/// watching.
///
/// The admin console's <c>CryptoSmithX.WebApp.Live.LiveNotifier</c> is the same idea and it is a
...
///
/// It is a copy rather than shared code because the original lives inside the WebApp application
...
/// LISTEN/NOTIFY is not table access, so <c>arena_reader</c> — a role that may select from eleven
...
public sealed class LiveNotifier
{
    ...
    private async Task PumpAsync(Task previous, CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                _logger.LogInformation("Arena live: listening on '{Channel}'", Channel);
                SetListening(true);
            ...
            {
                SetListening(false);
                _logger.LogWarning(ex, "Arena live: connection lost, retrying in {Backoff}", backoff);
            }
    ...
    private void OnNotification(object sender, NpgsqlNotificationEventArgs args)
    {
        ...
        {
            _logger.LogWarning("Arena live: malformed payload '{Payload}'", args.Payload);
        }
    }
    ...
    private void Publish(LiveEvent e)
    {
        ...
        foreach (var handler in handlers)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogWarning(ex, "Arena live: subscriber threw");
            }
        }
    }
