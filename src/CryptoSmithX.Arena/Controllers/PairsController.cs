...
namespace CryptoSmithX.Arena.Controllers;
...
public sealed class PairsController : Controller
{
    ...
    private readonly ArenaCache _cache;
    ...
    /// <summary>The list of pairs, and the site's front door: PathBase makes this /arena.</summary>
    [HttpGet]
    public async Task<IActionResult> Index(string? q, CancellationToken ct)
    {
        ...
        // on entries and a ceiling on key length, both argued on ArenaCache — and NOT, as this
        ...
        // exchange_instrument, on two columns (ArenaStore.PairsSql), and because a distinct term is
        ...
        var pairs = await _cache.GetAsync(
            ...
            async token =>
            {
                ...
                return await ArenaStore.ListPairsAsync(conn, search, token);
            },
            ct);
    ...
    /// <summary>
    /// One pair across every venue that lists it. Reached at /arena/BTC/USD — two bare segments,
    ...
    public async Task<IActionResult> Pair(string baseFamily, string quoteFamily, CancellationToken ct)
    {
        ...
        // /arena/Pairs/Pair?baseFamily=… through the default route, where no constraint applies. The
    ...
    private async Task<PairPageModel?> LoadAsync(string baseFamily, string quoteFamily, CancellationToken ct)
    {
        var data = await _cache.GetAsync<PairData?>(
            ...
            {
                await using var conn = await _db.OpenAsync(token);
                var comparison = await ArenaStore.GetPairAsync(conn, baseFamily, quoteFamily, token);
    ...
    public async Task Live(string baseFamily, string quoteFamily, CancellationToken ct)
    {
        ...
        // caller could open one on any string at all through /arena/Pairs/Live?baseFamily=….
        if (!PairAddress.IsFamily(baseFamily) || !PairAddress.IsFamily(quoteFamily))
        {
            Response.StatusCode = StatusCodes.Status404NotFound;
            return;
        ...
        try
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                catch (Exception ex) when (ex is not OperationCanceledException)
                {
                    ...
                    // (ArenaCache), so the retry is a real retry.
                    _logger.LogWarning(ex, "Arena live: rebuilding {Pair} failed", baseFamily + "/" + quoteFamily);
                    stalled = true;
                    await SyncSignalAsync();
                    continue;
                }
                ...
                try
                {
                    await Task.Delay(ArenaCache.Ttl, ct);
                }
                catch (OperationCanceledException)
                {
                    break;
