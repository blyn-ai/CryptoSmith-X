...
public static class PairPageLoader
{
        ...
        Db db, StudioCache cache, TimeProvider clock, string baseFamily, CancellationToken ct)
    {
        var data = await cache.GetAsync<PairData?>(
            ...
            async token =>
            {
                ...
                // The pool behind a vault-backed venue: one row per venue, not per pair.
            ...
            now)
        {
            ...
            VaultStates = data.VaultStates,
            Tape = data.Tape,
            Coverage = data.Coverage,
            Gaps = data.Gaps,
            Stress = data.Stress,
            Modes = data.Modes,
            TapePeakPerSecond = data.TapePeak,
        };
    }
    ...
    private sealed record PairData(
        ...
        IReadOnlyDictionary<string, VaultStateRow> VaultStates);
}
