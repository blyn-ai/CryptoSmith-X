...
(function () {
  ...
  // Таблица по выбранному полю больше не сортируется: порядок строк задан сервером — котировка
  // (USD, USDT, USDC, прочие), перп раньше спота, площадка по имени — и линия между котировками.
  // Строки, переезжающие при выборе поля ниже, ломали бы эти блоки (V2RowOrder).
