...
// Sentry: errors go to the shared cryptosmith-x project, tagged by component so hub/api/webapp-admin/studio
...
// WebApp.Admin's, unchanged, because the proxy in front of both is the same one. Studio has no cookie to
...
// studio_reader, not marketdata: the connection string on a public surface must not be able to
...
builder.Services.AddSingleton<StudioCache>();
...
// with the process. The WebApp.Admin mounts a volume instead because losing its keys logs every admin out.
//
...
if (!string.IsNullOrWhiteSpace(sentryDsn))
{
    SentrySdk.ConfigureScope(scope => scope.SetTag("component", "studio"));
}
...
// Studio lives under /studio, and the prefix is stripped HERE — in the app — and nowhere else.
...
// segments — /studio/BTC/USD — and a "{baseFamily}/{quoteFamily}" route registered above this line
// matches /studio/Pairs/Index too, swallowing every conventional controller URL on the site into a
...
// The pair page, addressed by the two halves of the folded pair: /studio/BTC/USD. BELOW the default
...
// two segments at all, so every stray request — /studio/.well-known/security.txt, a scanner's
// /studio/wp-admin/setup — becomes a database lookup for a pair by that name, run by an anonymous
...
// endpoint has been selected — so a two-segment route that accepted dots matched /studio/ds/styles.css
...
// into the route. In .NET `$` also matches immediately before a trailing newline, so /studio/BTC%0A/USD
...
// either at the root (studio.css, the two scripts) or three segments deep (ds/…, vendor/…), and
...
// the same action as /studio/Pairs/Pair?baseFamily=…, binding from the query string, with no
