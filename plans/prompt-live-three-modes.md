...
Работа режется на пять слайсов, **каждый — свой коммит в `main`**, в порядке A → B → C → D → E.
Никаких PR и веток: push в `main` → CI → тест-хост сходится сам; прод — только
`workflow_dispatch`. A и B независимы друг от друга. C ждёт A. D ждёт C. E — после всего.
Следующий слайс не начинать, пока предыдущий не прошёл свою «Приёмку» **на тест-хосте**.
