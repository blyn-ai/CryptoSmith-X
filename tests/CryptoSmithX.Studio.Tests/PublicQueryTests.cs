...
public sealed class PublicQueryTests
{
    ...
    {
        { nameof(StudioStore.PairsSql), StudioStore.PairsSql },
        { nameof(StudioStore.PairVenuesSql), StudioStore.PairVenuesSql },
    ...
    public void No_query_reads_a_table_the_public_role_cannot_see(string name, string sql)
    {
        ...
        // production — it fails outright under studio_reader, and it should fail here first.
        //
    ...
    public void The_fold_is_a_left_join_read_through_coalesce_and_never_an_inner_one()
    {
        ...
        Assert.Contains("left join asset_family_member bm", StudioStore.PairsSql, StringComparison.Ordinal);
        Assert.Contains("left join asset_family_member qm", StudioStore.PairsSql, StringComparison.Ordinal);
        Assert.Contains("coalesce(bm.family_code, i.base_asset)", StudioStore.PairsSql, StringComparison.Ordinal);
        Assert.Contains("coalesce(qm.family_code, i.quote_asset)", StudioStore.PairsSql, StringComparison.Ordinal);
    }
    ...
    public void An_unfolded_asset_is_still_addressable_as_its_own_family()
    {
        ...
        Assert.Contains(
            ...
            StudioStore.PairVenuesSql, StringComparison.Ordinal);
    }
    ...
    {
        Assert.Contains("left join market_snapshot_latest s", StudioStore.PairVenuesSql, StringComparison.Ordinal);
    }
    ...
    public void No_age_is_computed_in_sql()
    {
        ...
        Assert.DoesNotContain("now()", StudioStore.PairVenuesSql, StringComparison.Ordinal);
        Assert.DoesNotContain("current_timestamp", StudioStore.PairVenuesSql, StringComparison.Ordinal);
    }
}
