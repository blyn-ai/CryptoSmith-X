...
public sealed class PublicSurfaceTests
{
    ...
    public async Task A_flood_of_distinct_keys_cannot_grow_the_table_without_limit()
    {
        ...
        var cache = new StudioCache(new FakeClock());

        for (var i = 0; i < StudioCache.MaxEntries * 20; i++)
        {
            Assert.Equal(i, await cache.GetAsync("pairs:" + i, _ => Task.FromResult(i), CancellationToken.None));
        }

        Assert.True(cache.Count <= StudioCache.MaxEntries, $"held {cache.Count} entries");
    }
    ...
    public async Task A_key_the_cache_refuses_still_gets_its_answer()
    {
        ...
        for (var i = 0; i < StudioCache.MaxEntries * 3; i++)
        {
            flood = await cache.GetAsync("pairs:" + i, _ => Task.FromResult(i), CancellationToken.None);
        }

        Assert.Equal(StudioCache.MaxEntries * 3 - 1, flood);
    }
    ...
    public async Task A_flood_does_not_push_out_the_page_real_visitors_are_sharing()
    {
        ...
        for (var i = 0; i < StudioCache.MaxEntries * 5; i++)
        {
    ...
    public async Task A_key_too_long_to_hold_is_answered_and_not_remembered()
    {
        ...
        var cache = new StudioCache(new FakeClock());
        var key = "pairs:" + new string('A', StudioCache.MaxKeyLength);
        var loads = 0;

        Task<int> Load(CancellationToken _) => Task.FromResult(Interlocked.Increment(ref loads));

        Assert.True(key.Length > StudioCache.MaxKeyLength);
    ...
    public void A_long_string_is_refused_by_length_and_not_by_luck()
    {
        ...
        // only then returned 404, through /studio/Pairs/Pair?baseFamily=… where no route constraint
    ...
    public void Both_routes_and_the_action_check_carry_the_same_rule()
    {
        ...
        // /studio/ds/styles.css; PairAddress keeps the query-string address off the database. They
    ...
    public void The_pair_list_is_limited()
    {
        ...
        Assert.Contains("limit @limit", StudioStore.PairsSql, StringComparison.Ordinal);
    }
    ...
    public void The_limit_comes_with_the_count_it_cut()
    {
        ...
        Assert.Contains("count(*) over ()::int          as \"Matching\"", StudioStore.PairsSql, StringComparison.Ordinal);
        Assert.True(
            StudioStore.PairsSql.IndexOf("count(*) over ()", StringComparison.Ordinal)
            < StudioStore.PairsSql.IndexOf("limit @limit", StringComparison.Ordinal));
    }
